...
void video_display(VideoState *is) {
	...
	if (vp->m_pFrame){
		...
		t_new_w = t_window_w;
		t_new_h = t_window_h;
		int t_windowShould_w = ((int)rint(t_window_h * aspect_ratio)) & -3;
		if (t_windowShould_w > t_window_w)
		{
			t_new_w = t_window_w;
			t_new_h = ((int)rint(t_window_w / aspect_ratio)) & -3;;
		}
...
int main(int argc, char *argv[]) {
	...
	m_pWindow = SDL_CreateWindow("test windows", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 640, 480, SDL_WINDOW_SHOWN);
	if (!m_pWindow)
	{
